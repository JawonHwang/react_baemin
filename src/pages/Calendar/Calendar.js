...
const Calendar = () => {
  ...
  //   startDate.setDate(startDate.getDate() + 1); // 시작 날짜가 하루 뒤로 설정됨
  //   const NewStart = startDate.toISOString().split('T')[0] ? startDate.toISOString().split('T')[0] : null;
  
  //   // 종료 날짜 처리 (end가 없다면 start 날짜와 동일하게 설정)
  //   let NewEnd = null;
  //   if (event.end) {
  //     const endDate = new Date(event.end);
  //     endDate.setDate(endDate.getDate()); // 종료 날짜의 날짜를 하루 더해주지 않음
  //     NewEnd = endDate.toISOString().split('T')[0];
  //   } else {
  //     // 종료 시간이 없으면 시작 날짜와 동일하게 설정
  //     NewEnd = NewStart;
  //   }
  ...
  //   // axios를 통해 업데이트
  //   axios.put(`/api/calendar/${event.extendedProps.calId}`, {
  //     calStartTime: NewStart,
  //     calEndTime: NewEnd,
  //   }).then((res) => {
  //     refreshList();
  //   }).catch((error) => {
